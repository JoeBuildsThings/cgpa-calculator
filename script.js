...
function addCourse(semesterId) {
  ...
  renderSemesters();
  renderSummary();
  // focus the new course name so the keyboard opens straight away on phones
  const inputs = document.querySelectorAll('[data-sem="' + semesterId + '"] .c-name');
...
/* Rendering. Semesters rebuild on structure changes only. Typing and grade taps update numbers in place. */
...
function renderSemesters() {
  ...
  if (state.semesters.length === 0) {
    box.appendChild(el("div", { class: "sem" }, [el("p", { class: "empty", text: "No semesters yet. Add one, then tap your grades." })]));
    return;
  }
...
function renderSemester(s) {
  const gpa = el("span", { class: "sem-gpa", id: "gpa-" + s.id, text: "GPA " + semesterGPA(s).toFixed(2) });
  ...
  const wrap = el("div", { class: "sem", "data-sem": s.id }, [el("div", { class: "sem-head" }, [name, gpa, del])]);

  if (s.courses.length === 0) wrap.appendChild(el("p", { class: "empty", text: "No courses yet." }));
  for (const c of s.courses) wrap.appendChild(renderCourse(s, c));

  const add = el("button", { class: "add-course", type: "button", text: "+ Add course" });
...
function renderCourse(s, c) {
  const name = el("input", { class: "c-name", type: "text", value: c.name, placeholder: "Course code, e.g. LIS 201", maxlength: "60", "aria-label": "Course name" });
  name.addEventListener("input", (e) => {
    c.name = e.target.value;
    saveState();
  });

  const unit = el("input", { class: "c-unit", type: "number", inputmode: "numeric", min: "0", max: String(MAX_UNIT), step: "1", value: String(c.unit), "aria-label": "Credit units" });
  ...
  const del = el("button", { class: "x c-x", type: "button", "aria-label": "Remove course", text: "\u2715" });
  del.addEventListener("click", () => removeCourse(s.id, c.id));

  const grades = el("div", { class: "grades", role: "group", "aria-label": "Grade" });
  for (const g of Object.keys(GRADE_POINTS)) {
    const b = el("button", { class: "g", type: "button", "data-g": g, "aria-pressed": String(g === c.grade), text: g });
    b.addEventListener("click", () => {
      c.grade = g;
      for (const other of grades.children) other.setAttribute("aria-pressed", String(other.dataset.g === g));
      saveState();
      updateNumbers(s);
    });
  ...
  return el("div", { class: "row" }, [name, unit, grades, del]);
}
...
function renderSummary() {
  ...
  $("bar-fill").style.width = Math.min(100, (o.cgpa / 5) * 100) + "%";
  renderPlan();
}
...
function renderPlan() {
  ...
  if (needed > 5) {
    ...
    out.textContent = "";
    out.append("You need a GPA of ");
    out.appendChild(el("strong", { text: needed.toFixed(2) }));
    out.append(" over your next " + next + " units to reach " + target.toFixed(2) + ".");
  }
}
...
function init() {
  ...
  if (state.prior.cgpa !== "" || state.prior.units !== "") $("prior-drawer").open = true;
